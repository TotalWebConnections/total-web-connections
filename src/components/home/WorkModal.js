...
const WorkModal = ({ active, onClose, activeProject }) => {
  return (
    <div
      className={`WorkModal fixed h-full w-full flex justify-center items-center ${
        active ? "active" : null
      }`}
    ...
    >
      <div className="WorkModal__inner h-3/4 bg-white w-9/12 p-4 rounded-md overflow-scroll">
        <p
          className="text-right text-bold cursor-pointer transition hover:text-primary text-bold text-xl"
        ...
        <div class="grid md:grid-cols-2 gap-4 pt-4">
          <div>
            ...
            <div className="pt-6">
              <a
                className="text-primary hover:text-black transition-all"
          ...
          <div>
            <img
              src={`/images/portfolio/${activeProject?.image}`}
              width="90%"
            />
          </div>
        </div>
